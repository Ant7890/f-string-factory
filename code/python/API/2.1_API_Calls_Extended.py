...
def generate_password():
    ...
    for i in range(101):
        time.sleep(0.05)
        progress['value'] = i
        root.update_idletasks()  # Refresh GUI without entering mainloop

        if i == 0 or i % 60 == 0:  # Fetch a new quote immediately, then every ~3 seconds
            cycle_quotes()

    quote_var.set("")  # Clear quote when done
